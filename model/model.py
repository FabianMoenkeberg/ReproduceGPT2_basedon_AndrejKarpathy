...
@dataclass
class GPTConfig:
    block_size: int = 1024  # maximum sequence/context length
    vocab_size: int = 50257  # number of tokens in the vocabulary: 50000 + 256 bytes + 1 endof-sequence token. Note the 256 byte tokens are used to ensure that each byte can be tokenized. (BPE is tokenizing on the byte level, thus remaining bytes could be handled here)
...
class CausalSelfAttention(nn.Module):
    ...
    This is the communication layer of the transformer block.
    It computes the attention scores and applies them to the values."""
    def __init__(self, config):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # is nh * hs
        # e.g. in GPT-2 with n_head=12 and nhs=64, we have C = nh*hs = 768
        qkv = self.c_attn(x)
        q, k, v = qkv.split(self.n_embd, dim=2)
        # Note, we reshape the tensors to have the shape (B, T, nh, hs) such that we treat the vectors as a tensor with self.n_head*B batches (both are treated as batches).
        # -> This results that the following operations are done in parallel for all heads.
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.attn = CausalSelfAttention(config)  # This is the communication layer
        self.ln_2 = nn.LayerNorm(config.n_embd)
        self.mlp = MLP(config)  # This is the mapping layer, no communication here.
